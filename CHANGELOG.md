...
四项 v0.1.15 deferred 后续打包成单 PR：framework-review B5 三跳深化、HOOKS_MANIFEST 全量化、Pydantic V2 strict mode（内部 state 模型）、scaffold 镜像彻底消除（hatch force-include + editable 回退）。底层把"框架自审"从 5 个子检查扩到 9 个，并消除了一个长期需双写维护的镜像目录。
