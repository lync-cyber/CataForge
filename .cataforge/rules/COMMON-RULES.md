...
| UNATTENDED_STAGNATION_THRESHOLD | 3 | 连续 N 轮无进展（git HEAD 未变 且 无新事件）→ 循环级熔断 | unattended-building-loop |
...
| UNATTENDED_SAME_ERROR_CEILING | 5 | 连续 N 轮命中同一错误签名（root cause 指纹不变，即便 HEAD 在变）→ 同错熔断 | unattended-building-loop |
...
| TDD | 默认 light（RED+GREEN 合并）；升 standard 条件见 `TDD_DEFAULT_MODE`，REFACTOR 按 `TDD_REFACTOR_TRIGGER` 条件触发 | RED+GREEN 合并（`tdd_mode: light`），REFACTOR 触发同 standard | implementer 主线程一次性写测试+实现，跳过 RED/GREEN/REFACTOR 子代理调度 |
...
| approved | 审查通过，无问题 | reviewer | 见 §verdict_blocking_semantics |
| approved_with_notes | 审查通过但有 MEDIUM/LOW（无 CRITICAL/HIGH） | reviewer | 见 §verdict_blocking_semantics |
| needs_revision | 审查不通过（有 CRITICAL/HIGH） | reviewer | 见 §verdict_blocking_semantics |
...
**修改时**（默认不写变更说明）：
- 禁止版本里程碑（"v0.4.0+ 新增"、"自 vX.Y.Z 起"、"MVP 阶段"）、阶段标签（"先 MVP 后升级"、"以后再做"）、对比叙事（"原方案 X、改为 Y"、"不再使用 X"）。<!-- allow-design-residue: 规则定义本身需引用被禁短语作为示例 -->
