...
GREEN/Light 完成前对让测试 PASS 的断言做一次强度自检。弱断言判据以 test-writer §Execution Rules 行为断言强制 的禁止模式表为单一事实来源（结构存在性断言、非空 / 类型 / 可调用单立断言、仅校验 mock / spy / stub 调用计数等）。额外补一类 GREEN 专属信号：mock 出现"诡异条件"（永远返回常量 / 永远 raise / 强行短路真实路径）让测试 PASS，视为 implementation bug 假阳性。

处置：GREEN/dispatch 模式发现弱断言 → 返回 blocked，`<questions>` 列出薄弱测试位置要求重新生成；light 模式自己写时 → 直接重写，断言绑定到可观测属性（接口契约定义的返回值字段 / 被测对象的状态变化 / 对外可见副作用：日志 / 持久化 / 事件发布 / IO）后再 PASS。
