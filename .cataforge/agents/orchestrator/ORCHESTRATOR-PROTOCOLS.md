...
6. **填入 §执行环境 + 最小 permissions** — 按顺序运行两条命令:
   ...
   - `python .cataforge/scripts/framework/setup.py --apply-permissions`：根据技术栈最小化 `.claude/settings.json` 的 `permissions.allow`，裁掉未使用的 Bash 白名单条目。
...
- **[EVENT]** `python .cataforge/scripts/framework/event_logger.py --event review_verdict --phase {当前阶段} --agent reviewer --status needs_revision --detail "审查不通过，需修订"`

当文档状态为 needs_revision 时（orchestrator 侧职责）:
1. **[EVENT]** 记录修订开始:
   ```bash
   python .cataforge/scripts/framework/event_logger.py --event revision_start --phase {当前阶段} --agent {原Agent} --detail "进入修订流程 needs_revision(N)"
   ```
...
1. **[EVENT]** 记录审查结论:
   ```bash
   python .cataforge/scripts/framework/event_logger.py --event review_verdict --phase {当前阶段} --agent reviewer --status approved_with_notes --detail "审查通过但有建议"
   ```
...
4. **[EVENT]** 记录用户决策:
   ```bash
   python .cataforge/scripts/framework/event_logger.py --event user_decision --phase {当前阶段} --detail "用户选择: {接受并继续|要求修复}"
   ```
...
5. **[EVENT BATCH]** 通过 `--batch` 单次 stdin 管道一次性记录 4 条事件（phase_end → review_verdict → state_change → phase_start）:
   ```bash
   python .cataforge/scripts/framework/event_logger.py --batch <<'EOF'
...
1. **[EVENT]** 记录异常事件:
   ```bash
   python .cataforge/scripts/framework/event_logger.py --event incident --phase development --status rolled-back --detail "REFACTOR rolled-back，使用 GREEN 产出"
   ```
...
2. **[EVENT]** 记录跳过事件:
   ```bash
   python .cataforge/scripts/framework/event_logger.py --event review_verdict --phase development --agent orchestrator --status approved --detail "sprint-review skipped (micro sprint)"
   ```
...
- **[EVENT] 手动**: 使用 `python .cataforge/scripts/framework/event_logger.py` CLI，已嵌入各协议步骤中（标记为 **[EVENT]**）

```bash
python .cataforge/scripts/framework/event_logger.py --event phase_start --phase architecture --detail "进入架构设计阶段"
```
