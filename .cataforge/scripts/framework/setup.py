...
用法:
  python .cataforge/scripts/framework/setup.py               # 完整安装检测
  python .cataforge/scripts/framework/setup.py --with-penpot  # 含 Penpot MCP 安装
  python .cataforge/scripts/framework/setup.py --check-only   # 仅检测，不做任何修改
...
FRAMEWORK_CORE_PERMISSIONS = [
    ...
    "Bash(python .cataforge/integrations/penpot/setup_penpot.py --ensure)",
]
...
def _penpot_show_menu() -> str:
    ...
    print(f"      {DIM}  python .cataforge/scripts/framework/setup.py --with-penpot{NC}")
    print()
            ...
            if ans == "a":
                ...
                if start_docker_desktop(timeout=120):
                    ...
                    if ans2 in ("y", "yes"):
                        ...
                        print(
                            f"    {DIM}python .cataforge/scripts/framework/setup.py --with-penpot{NC}"
                        )
                        return False
            elif ans == "m":
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "示例:\n"
            "  python .cataforge/scripts/framework/setup.py               # 完整安装检测\n"
            "  python .cataforge/scripts/framework/setup.py --with-penpot  # 含 Penpot MCP\n"
            "  python .cataforge/scripts/framework/setup.py --check-only   # 仅检测\n"
        ),
