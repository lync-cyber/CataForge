...
Usage:
  python .cataforge/scripts/framework/upgrade.py local /path/to/new [--dry-run] [--backup-dir <dir>]
  python .cataforge/scripts/framework/upgrade.py check [--repo owner/repo] [--url URL] [--branch main]
  python .cataforge/scripts/framework/upgrade.py upgrade [--repo owner/repo] [--url URL] [--dry-run]
  python .cataforge/scripts/framework/upgrade.py verify
...
def cmd_check(args):
    ...
    print("\n可运行以下命令升级:")
    print("  python .cataforge/scripts/framework/upgrade.py upgrade --dry-run  # 预览变更")
    print("  python .cataforge/scripts/framework/upgrade.py upgrade             # 执行升级")
    sys.exit(0)
...
def cmd_upgrade(args):
    ...
    if exit_code == 0 and not args.dry_run:
        ...
        print("\n已记录升级状态到 .cataforge/framework.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "示例:\n"
            "  python .cataforge/scripts/framework/upgrade.py local /path/to/new --dry-run\n"
            "  python .cataforge/scripts/framework/upgrade.py check --repo owner/CataForge\n"
            "  python .cataforge/scripts/framework/upgrade.py upgrade --repo owner/CataForge\n"
            "  python .cataforge/scripts/framework/upgrade.py verify\n"
        ),
