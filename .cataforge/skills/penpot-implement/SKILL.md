...
## Anti-Patterns
- 禁止: 用 Penpot 覆盖 ui-spec 的语义契约 —— 组件身份/Props/状态枚举/AC 绑定以 ui-spec 为权威源；视觉实值（尺寸/真实 CSS/层级几何）以当前 authoring surface 的视觉权威源为准（Penpot-first 取 Penpot、doc-first 取 ui-spec 视觉段）；语义冲突以 ui-spec 为准，避免视觉漂移绕过 reviewer 的语义校对
