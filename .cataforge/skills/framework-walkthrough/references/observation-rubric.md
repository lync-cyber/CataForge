...
| 一致性门 | C-5b~C-5f | Phase Transition 门禁链（`cataforge phase transition`）的 stale-deps / reconcile / doc-consistency / claude-md 门是否逐门执行、阻塞类门是否真阻塞 | hygiene 越界却 WARN 放行、doc-consistency 在 Phase 2+ 未触发、事件批出现半截状态或重跑重复落盘 |
