...
- 真实输入路径声明 — e2e 套件至少含一处真实交互调用（键入 / 填充 / 点击，各框架 API 见 [`docs/reference/test-and-e2e-apis.md`](../../../docs/reference/test-and-e2e-apis.md)），无任何 → WARN（提示套件可能纯 fixture 注入）
...
## 测试套件性能纪律
测试套件随项目增长，执行时长直接决定开发反馈循环；规划/编写时按下列纪律保持其可持续：

- **分层标签隔离慢测**：慢测（构建产物 / 起容器 / 外部进程 / 网络）与集成测试打分类标签（marker / tag / 目录约定），使"纯单元"内循环可只跑快测；标签缺失会让一次全量跑成为唯一选项，套件越大越不可用
- **昂贵确定 setup 跨用例复用**：构建一次即确定的环境（已构建产物 / 已初始化的数据存储 / 预置 fixture 数据）用 session/module 级 fixture 构建一次复用，不每测重建；每测各自重建是套件随测试数线性变慢的首要根因
- **进程内优先**：可进程内验证的逻辑不 spawn 子进程 / 起服务；子进程仅用于验证真实安装 / CLI / 跨进程边界，并随其打慢测/集成标签
- **并行就绪**：测试相互隔离（不共享可变全局 / 不依赖执行顺序），使 test runner 支持并行时可直接按核数并行
