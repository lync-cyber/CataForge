...
- 能做: 指导orchestrator编排TDD三阶段子代理(RED/GREEN/REFACTOR)、light/standard 档位路由、prototype 主线程内联实现、同 sprint_group 独立任务并行调度、定义子代理prompt模板
- 不做: 需求分析、架构设计、文档生成
...
- orchestrator 自身在主线程使用 Step 1 已提取的上下文，按 light 模式的"先测试后实现"步骤直接产出 test_files + impl_files
- 跳过 Step 4 REFACTOR
