...
suggested-tools: file_read, file_write, file_edit, file_grep, shell_exec
...
7. 按依赖关系划分Sprint(参考 task-dep-analysis 输出的 sprint_groups)，遵循 Sprint 切分原则:
...
## Anti-Patterns
- 禁止: 单任务预估 LOC > `TASK_SPLIT_LOC` 或 AC > 6 条而不拆 —— 超此尺寸任务粒度不再单一，应在 task-decomp 阶段先拆
...
- 禁止: AC 仅描述"实现 X 功能"/"支持 Y 格式" —— 模糊 AC 导致 test-writer 退化为存在性检查，无法推导断言期望值
- 禁止: AC 只覆盖被引用 `arch#§N.API-xxx` 契约的正常路径 —— 如契约声明 `401 E_AUTH` + `403 E_PERMISSION_DENIED` 两条安全路径，AC 只测正常返回，缺口要到下游 code-review 标 HIGH 或 pre-wiring 审计才暴露
