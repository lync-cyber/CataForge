...
[NAV]
- §1 架构概览 → §1.1 项目类型, §1.2 架构风格, §1.3 系统上下文图, §1.4 技术栈, §1.5 外部消费边界
- §2 模块划分 → M-001..M-{NNN}
...
|------|------|------|----------|----------|----------|
| {如 后端} | {如 Python} | {如 3.12} | {Stable/LTS} | {为何选它、对比备选，勿留空} | {tech-eval 引用} |

### 1.5 外部消费边界 (external_oracles)
{产物契约依赖第三方外部系统消费时逐条声明；无外部消费方写 N/A。机制契约见 `.cataforge/references/external-truth-first.md`}

| ID | 外部系统 | 对产物的变换行为 | 验证方式 |
|----|---------|----------------|---------|
| EO-001 | {系统名} | {如 粘贴过滤剥离样式 / 导入重排结构} | {经真实通道的端到端比对方式} |
...
- **对外接口**: API-001, API-002 (引用 §3 接口契约)
- **依赖模块**: M-002, M-005
- **内部关键组件**: {类/组件列表}
