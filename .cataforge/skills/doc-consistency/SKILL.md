...
**关系类检查后端无关（自动）**: AC 追踪检查（`check_prd_arch_ac_coverage` / `check_prd_devplan_ac_traceability`）由框架按当前可用的最高保真后端执行——高保真后端按实体关系判定,消除字面 ID 在代码块 / HTML 注释 / 跨分卷出现上的假阳性 / 假阴性;其余检查（NFR 映射、优先级对齐、API 端点契约、孤立组件）依文档结构而非实体关系。后端不可达时自动降级,agent 无需感知。
