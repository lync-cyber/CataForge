...
- **consistency** — 跨文档语义一致性校验，PRD↔ARCH AC 追踪、ARCH↔DEV-PLAN API 契约、PRD↔UI-SPEC 用户可见性覆盖；输出 F-NNN 追踪矩阵 + 严重等级问题清单；退出码 0 通过（含仅 MEDIUM/LOW advisory）/ 1 存在 CRITICAL/HIGH（2 保留给坏参数，不由 findings 产生），由 Phase Transition Protocol 门禁链（`cataforge phase transition` 的 doc-consistency 门）在 Phase 2+ 转换时调用；Layer 1 由内置引擎 `cataforge skill run doc-consistency` 支撑
