...
sed -i 's/"platform": "cursor"/"platform": "claude-code"/' .cataforge/framework.json
cataforge deploy
```

为什么：`setup --platform` 同时刷新 `runtime.platform` 与 `.deploy-state` 平台戳记，并触发该平台的 profile 校验。绕过 `setup` 直接改 JSON 会让 `.deploy-state` 与实际产物不同步——下一次 `bootstrap` 会因为戳记一致而跳过 deploy，旧 IDE 的产物留在仓库里，新 IDE 反而没有产物。

正确做法见上方 `cataforge setup --platform <id>`。
