...
after（1 个父 skill，发现面 1 条 description）
  .cataforge/skills/context/
    SKILL.md                  # 小调度体：上下文 I/O 全景 + "按需 load 哪个 branch"
    references/
      navigate.md             # ← doc-nav 正文（ReadPort：按 ref 取确定章节/实体）
...
| 文件:行 | 现状 | 改为 |
|---|---|---|
| `runtime/skill/loader.py:17` | `"doc_review": "doc-review"`（`_BUILTIN_ID_MAP`） | 删除该行；builtin 目录 `context` 经 `_BUILTIN_ID_MAP.get(name, name)` 回退即得 id `context` |
| `runtime/skill/loader.py:35` | `_BUILTIN_EVENT_LOGGED` 含 `"doc-review"` | 改为 `"context"`（review/consistency 两脚本的 EVENT-LOG ref 已带 `skill:context/<script>` 区分；consistency 由此也进入日志，为有意的可观测性增益） |
| `runtime/skill/builtins/doc_review/`、`doc_consistency/` | 两个独立 builtin 包 | 合并为 `builtins/context/`：入口脚本 `review.py` / `consistency.py`（保留 `__main__` guard），helper 模块平移；修内部 import 前缀 `...builtins.doc_review`/`doc_consistency` → `...builtins.context` |
| `runtime/skill/builtins/framework_review/checks/b3.py:75` | `"doc-review": "cataforge.runtime.skill.builtins.doc_review"` | `"context": "cataforge.runtime.skill.builtins.context"`（B3 按 skill-id 校验 builtin 模块路径） |
| `runtime/skill/builtins/framework_review/_constants.py:32,51` | `B1_REQUIRED_SECTIONS_EXEMPT_SKILLS` / `ORPHAN_SKILL_WHITELIST` 含 `"doc-nav"`、`"doc-gen"` | 改为 `"context"`（父 skill 被各 agent 引用后，孤儿白名单项可同时移除多数条目） |
| `runtime/skill/builtins/doc_review/template_registry.py:40,49,99` | 硬编码 `.cataforge/skills/doc-gen/templates` | `.cataforge/skills/context/templates`（模板随 generate 分支迁入 `context/`） |

### 6.3 引用站点改动（breadth，按类别）

blast-radius 实测约 360 处、80+ 文件。五个旧 id 统一收敛到 `context`（运行时脚本另带 `--script <branch>`）。按类别批改：

| 类别 | 量级 | 改法 |
|---|---|---|
| SKILL.md `depends:`（`loader.py` 解析） | 18 个依赖 `doc-nav`、7 个依赖 `doc-gen`、2 个依赖 doc-review/consistency 链 | 统一改为 `depends: [context]`，去重（如 task-decomp `[doc-gen, doc-nav, task-dep-analysis]` → `[context, task-dep-analysis]`） |
...
**保留不改的**：常量名 `DOC_SPLIT_THRESHOLD_LINES` / `DOC_REVIEW_L2_SKIP_*`（`framework.json` + COMMON-RULES + checker + tests 多处引用，重命名是纯 churn、无澄清收益）；它们归属 `context` 的 review 分支即可，名字不动。
...
- **default script 行为**：`runner._find_script` 在不传 `--script` 时取 `meta.scripts[0]`（按文件名排序）。合并后 `consistency.py` 排在 `review.py` 前，默认会落到 consistency。须在 `context/SKILL.md` 与协议中明确两个 Layer 1 调用**始终显式带 `--script`**，避免依赖默认顺序。
- **EVENT-LOG 兼容**：`record_to_event_log` 现按 skill-id 生效；`context` 入集后 review 与 consistency 都记录，ref 形如 `skill:context/review`。reflector/sprint-review 若按旧 ref `skill:doc-review/...` 解析，需同步更新匹配前缀。
- **framework-review CHECKS_MANIFEST 对账**：`doc-review` 的 `CHECKS_MANIFEST`（COMMON-RULES 声明"与 manifest 不一致即 FAIL"）随包迁入 `builtins/context/`，B3 模块路径同改；迁移后跑一次 framework-review 确认 manifest 对账通过。
- **deploy 无机制改动**：`deploy_skills` 按 skill 子目录整树 copy+render（`skills.py:110`），`context/references/*` 与 `templates/` 自动随 `context/` 部署，无需新增逻辑。

### 6.6 验证矩阵

| 验证项 | 命令 | 通过判据 |
|---|---|---|
| 运行时入口 | `cataforge skill run context --script review -- <args>` / `--script consistency -- docs/` | 退出码与旧 `doc-review`/`doc-consistency` 一致 |
| 发现面 | `cataforge skill list` | 含 `context`；无 `doc-nav/gen/review/consistency`、`kg-ask` |
...
- **大面积重命名**：~360 处引用，遗漏即 dangling。缓解：S3 完成后用一次全仓 grep（旧五 id + `builtins.doc_review`/`doc_consistency`）断言零残留，并在 S5 落守卫固化。
- **默认 script 错位**：见 §6.5，靠"始终显式 `--script`"消除。
- **回滚单元**：S1–S5 各为独立可回滚提交；S1（runtime）与 S2（prompt 树）若需回退须成对回退，因二者共同决定 `context` id 能否解析。
