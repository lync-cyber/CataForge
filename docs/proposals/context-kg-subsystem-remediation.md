...
> 状态：支柱一（mode 收敛）+ 支柱二（ModePolicy / R-003 reconcile 门禁 / R-004 授权门 + source_doc 保持）+ Track C（R-010 模板 xref + 多 xref 主语绑定修复 / M4 write-doc relations=0 告警 / R-011 M 级覆盖经 `cf:realizes` 真正生效 / 对称 diff card 假阳性收敛）+ Track B（R-007 phase status --project-root / R-008 节名 / U2 status 输出 / M3+R-006 reconcile --json / C5 flag 命名 / C3 --project-root 统一 / D6 validate=index --strict --dry-run 单实现 / R-005 事件 phase 取被审 doc_type）+ Track D（走查协议 P-001~P-004）+ prompt 资产收敛 + §7 迁移（context.strategy/authoring→mode）已实现并通过全量测试；支柱三（命令分层 / Track A：context delete/update 门面、kg import/export 降隐藏 + reconcile→drift-check、read.py 纯函数化、逆向依赖归位、prompt↔CLI 防腐守卫含 superseded 黑名单）亦已落地并通过全量测试；§8 两份旧提案的取代标注已落实。全部三支柱与四轨完成。已接受的实现偏离（C7 用函数 `_require_graph_mode` + ModePolicy 单点代替 `@requires_mode` 装饰器；mode 缺失不读旧双轴推导而由 doctor flag + upgrade 重写）以代码为准。
