...
| 项目从零搭 SDLC 流程 | 内置 13 个 Agent + 31 个 Skill，覆盖需求 → 架构 → 设计 → TDD → 评审 |
...
CataForge 专为中文开发团队设计。13 个内置 Agent 的指令、31 个 Skill 的定义、全套 SDLC 文档模板（PRD、架构文档、开发计划等）均以中文撰写，Agent 之间通过中文语义传递上下文。你无需在英文 prompt 和中文需求之间反复翻译——输入中文需求，输出中文文档，代码注释和 commit 风格也遵循团队约定。
