...
- **`kg_auto_ingest` hook 在 4 个 platform 上的 degradation 配置缺失** —— PR #130 注册了 hook + builtin script + degradation_template，但漏写每平台 `profile.yaml` 的 `hooks.degradation:` 块；本次为 claude-code / cursor / opencode 补 `native`，为 codex 补 `degraded`（无 Edit|Write matcher）。修复 framework-review B6_hook_degradation_coverage 4 个 WARN。
