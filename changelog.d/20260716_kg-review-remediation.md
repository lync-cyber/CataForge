...
- **hash-skip home 同步纳入 phase 5 回滚** —— `WriteStats.home_synced` 记录已应用的槽位同步，批次异常回滚时反向恢复；stale Section 判定收敛为 `writer.stale_section_iris` 单一定义（ingest 清理与 authoring 替换共用）。
