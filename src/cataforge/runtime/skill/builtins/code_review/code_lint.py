...
Usage:
  python -m cataforge.runtime.skill.builtins.code_review.code_lint <file_or_dir> [--fix]
  python -m cataforge.runtime.skill.builtins.code_review.code_lint scan <path> \
...
def main() -> None:
    ...
    if sys.argv[1] == "scan":
        if len(sys.argv) < 3:
            print(
                "用法: python -m cataforge.runtime.skill.builtins.code_review.code_lint"
                " scan <path> [--focus <category[,category...]>]"
            )
            sys.exit(2)
        target = sys.argv[2]
