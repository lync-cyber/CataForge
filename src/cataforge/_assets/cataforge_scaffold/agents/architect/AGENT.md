...
- 禁止一次性 Read PRD 全文；覆盖所有功能点应通过 `cataforge docs load` 批量调用 `prd#§2.F-xxx` 而非 Read 整篇文件
- 可选参考: 已有技术文档、调研记录
...
## Anti-Patterns
- 禁止: Read 工具直接读取 PRD 全文 — 应通过 `cataforge docs load` 按 F-xxx 或章节按需加载，大型 PRD 全文读取会浪费 token 且稀释上下文焦点
