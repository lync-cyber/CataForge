...
connection; the legacy branch delegates to :mod:`cataforge.domain.docs.loader`
and :mod:`cataforge.domain.docs.indexer` so non-active doc_types continue to
...
from cataforge.domain.kg.facade import KnowledgeGraph
...
def _emit(name: str, replacement: str) -> None:
    warnings.warn(
        f"cataforge.domain.kg._shim.{name}() is deprecated since 0.5.0; "
...
def extract(
    ...
    *,
    ...
    Legacy branch: delegates to :func:`cataforge.domain.docs.loader.extract`
...
def _legacy_extract(
    ...
    from cataforge.domain.docs.loader import LoadSectionError
    from cataforge.domain.docs.loader import extract as _file_extract
...
def plan_load(
    ...
    *,
    ...
    Legacy path: delegates to :func:`cataforge.domain.docs.loader.plan_load`,
...
def _legacy_plan_load(items: list[str], budget: int, project_root: str | Path) -> dict[str, Any]:
    from cataforge.domain.docs.loader import plan_load as _file_plan_load
...
def build_full_index(
    *,
    ...
    :func:`cataforge.domain.docs.indexer.build_full_index` and re-shapes the
...
def _legacy_build_full_index(
    ...
    from cataforge.domain.docs.indexer import build_full_index as _file_build
...
def resolve_deps(
    ...
    *,
    ...
    Legacy path: delegates to
    :func:`cataforge.domain.docs.loader.resolve_deps` which walks
...
def _legacy_resolve_deps(item_id: str, project_root: str | Path) -> list[str]:
    from cataforge.domain.docs.loader import resolve_deps as _file_resolve
...
def extract_with_body(
    ...
    *,
    ...
    KG path: invokes `cataforge.domain.kg.export.render_entity()` on the
    resolved entity. Legacy path: the legacy `_legacy_extract` already
    returns the raw text in `body`.
    """
    _emit(
        "extract_with_body",
        "kg.query.entity() + cataforge.domain.kg.export.render_entity()",
    )
    cfg = _config_for(project_root, db_path=db_path, overrides=config)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
    ...
    if base.get("body") is not None:
        return base
    from cataforge.domain.kg.export import render_entity  # noqa: PLC0415
...
def legacy_validate_report(
    *,
    ...
    to :func:`cataforge.domain.docs.indexer.validate_docs`.
    """
...
def _kg_validate_report(
    ...
    *,
    ...
    from cataforge.domain.kg.reconcile import reconcile as _kg_reconcile  # noqa: PLC0415
    from cataforge.domain.kg.validate import validate as _kg_validate  # noqa: PLC0415
...
def _legacy_validate_report(
    ...
    from cataforge.domain.docs.indexer import validate_docs
...
def source_section(
    ...
    *,
    ...
    against the file resolved via `cataforge.domain.docs.loader.resolve_file`.
    """
...
def _legacy_source_section(
    ...
    from cataforge.domain.docs.loader import (  # noqa: PLC0415
        DocResolveError,
        resolve_file,
    )
