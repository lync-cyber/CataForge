...
1. 查阅注册表: `Read .claude/skills/doc-gen/templates/_registry.yaml`，找到 `{template_id}` 条目的 `path` 字段
   读取模板文件: `Read .claude/skills/doc-gen/templates/{path}`（如 `standard/prd.md`）
...
4. 更新机器索引: `python .claude/scripts/docs/build_doc_index.py --project-root . --doc-file {最终文档路径}`
5. **[EVENT]** `python .claude/scripts/framework/event_logger.py --event doc_finalize --phase {当前阶段} --ref "{doc_id}" --detail "文档finalize: {doc_id}"`
