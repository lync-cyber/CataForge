...
## Input Contract
- 必须加载: 通过 `python .claude/scripts/docs/load_section.py` 按 M-xxx / API-xxx 加载 `arch#§2.M-xxx` + `arch#§3.API-xxx` + `arch#§6` + `arch#§7`；按 C-xxx / P-xxx 加载 `ui-spec#§2.C-xxx` 和 `ui-spec#§3.P-xxx`
...
- 加载示例: `python .claude/scripts/docs/load_section.py arch#§2.M-001 arch#§3.API-001 ui-spec#§2.C-001 ui-spec#§3.P-001`
...
- 禁止: 依赖图存在循环
- 禁止: 修改ARCH中的技术决策
- 禁止: Bash 仅用于运行 `python .claude/skills/dep-analysis/scripts/dep_analysis.py` 或 `python .claude/scripts/docs/load_section.py`，禁止执行其他命令
