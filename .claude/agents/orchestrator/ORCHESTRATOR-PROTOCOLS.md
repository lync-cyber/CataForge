...
6. **填入 §执行环境 + 最小 permissions** — 按顺序运行两条命令:
   - `python .claude/scripts/setup.py --emit-env-block`：将输出注入 CLAUDE.md §执行环境 节以替换占位符。退出码 2 表示未检测到已知技术栈，此时将该节内容置为 `- 无自动检测到的标准包管理器（请根据实际技术栈手动填写）`。
   - `python .claude/scripts/setup.py --apply-permissions`：根据技术栈最小化 `.claude/settings.json` 的 `permissions.allow`，裁掉未使用的 Bash 白名单条目。
...
- **[EVENT]** `python .claude/scripts/event_logger.py --event review_verdict --phase {当前阶段} --agent reviewer --status needs_revision --detail "审查不通过，需修订"`

当文档状态为 needs_revision 时（orchestrator 侧职责）:
1. **[EVENT]** 记录修订开始:
   ```bash
   python .claude/scripts/event_logger.py --event revision_start --phase {当前阶段} --agent {原Agent} --detail "进入修订流程 needs_revision(N)"
   ```
...
1. **[EVENT]** 记录审查结论:
   ```bash
   python .claude/scripts/event_logger.py --event review_verdict --phase {当前阶段} --agent reviewer --status approved_with_notes --detail "审查通过但有建议"
   ```
...
4. **[EVENT]** 记录用户决策:
   ```bash
   python .claude/scripts/event_logger.py --event user_decision --phase {当前阶段} --detail "用户选择: {接受并继续|要求修复}"
   ```
...
1. **[EVENT]** 记录异常事件:
   ```bash
   python .claude/scripts/event_logger.py --event incident --phase development --status rolled-back --detail "REFACTOR rolled-back，使用 GREEN 产出"
   ```
...
- **[EVENT] 手动**: 使用 `python .claude/scripts/event_logger.py` CLI，已嵌入各协议步骤中（标记为 **[EVENT]**）
