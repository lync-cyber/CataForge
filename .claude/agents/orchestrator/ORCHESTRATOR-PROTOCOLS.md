...
| 事件 | 触发条件 | 写入方式 |
|------|---------|---------|
| session_start | 会话启动 | **Hook 自动** (session_context.py)，含 compact 去重 |
...
2. **[EVENT]** 如果存在明显偏差，记录纠偏事件:
   ```bash
   python .claude/scripts/event_logger.py --event correction --phase {当前阶段} --agent {agent_id} --detail "用户纠偏: {偏差类型}"
   ```
3. 如果存在明显偏差（用户选择了 agent 未预期的选项），追加记录到 docs/reviews/CORRECTIONS-LOG.md：
   ```
   ### {date} | {agent_id} | {phase}
