...
6. **填入 §执行环境 + 最小 permissions** — 按顺序运行两条命令:
   - `python .claude/scripts/framework/setup.py --emit-env-block`：将输出注入 CLAUDE.md §执行环境 节以替换占位符。退出码 2 表示未检测到已知技术栈，此时将该节内容置为 `- 无自动检测到的标准包管理器（请根据实际技术栈手动填写）`。
   - `python .claude/scripts/framework/setup.py --apply-permissions`：根据技术栈最小化 `.claude/settings.json` 的 `permissions.allow`，裁掉未使用的 Bash 白名单条目。
   本步骤的目的是让包管理器/安装命令/测试命令以项目指令形式固化到 CLAUDE.md，并收紧运行时权限以符合最小权限原则。
7. **创建 docs/NAV-INDEX.md** — 生成空索引骨架
...
> 子代理收到 `task_type=continuation` 后的恢复步骤见 `.claude/rules/SUB-AGENT-PROTOCOLS.md §task_type=continuation 恢复流程`，orchestrator 无需关注子代理内部执行细节。

## Revision Protocol
当 reviewer 返回 needs_revision 时，先记录审查结论:
- **[EVENT]** `python .claude/scripts/framework/event_logger.py --event review_verdict --phase {当前阶段} --agent reviewer --status needs_revision --detail "审查不通过，需修订"`

当文档状态为 needs_revision 时（orchestrator 侧职责）:
1. **[EVENT]** 记录修订开始:
   ```bash
   python .claude/scripts/framework/event_logger.py --event revision_start --phase {当前阶段} --agent {原Agent} --detail "进入修订流程 needs_revision(N)"
   ```
...
> 子代理收到 `task_type=revision` 后的修订步骤见 `.claude/rules/SUB-AGENT-PROTOCOLS.md §task_type=revision 修订流程`。

## Approved-with-Notes Protocol
当 reviewer 返回 approved_with_notes 时:
1. **[EVENT]** 记录审查结论:
   ```bash
   python .claude/scripts/framework/event_logger.py --event review_verdict --phase {当前阶段} --agent reviewer --status approved_with_notes --detail "审查通过但有建议"
   ```
...
4. **[EVENT]** 记录用户决策:
   ```bash
   python .claude/scripts/framework/event_logger.py --event user_decision --phase {当前阶段} --detail "用户选择: {接受并继续|要求修复}"
   ```
...
5. **[EVENT BATCH]** 通过 `--batch` 单次 stdin 管道一次性记录 4 条事件（phase_end → review_verdict → state_change → phase_start）:
   ```bash
   python .claude/scripts/framework/event_logger.py --batch <<'EOF'
...
1. **[EVENT]** 记录异常事件:
   ```bash
   python .claude/scripts/framework/event_logger.py --event incident --phase development --status rolled-back --detail "REFACTOR rolled-back，使用 GREEN 产出"
   ```
...
2. **[EVENT]** 记录跳过事件:
   ```bash
   python .claude/scripts/framework/event_logger.py --event review_verdict --phase development --agent orchestrator --status approved --detail "sprint-review skipped (micro sprint)"
   ```
...
> 子代理收到 `task_type=amendment` 后的修订步骤见 `.claude/rules/SUB-AGENT-PROTOCOLS.md §task_type=amendment 变更修订流程`。
...
- .claude/scripts/ — `lib/`（`_common` 等共享模块）、`framework/`（setup、upgrade、event_logger、phase_reader）、`docs/`（load_section、build_doc_index）；Penpot 脚本在 `.claude/integrations/penpot/setup_penpot.py`
...
### 初始化安装
- 运行: `python .claude/scripts/framework/setup.py` 检测环境并安装依赖
- 可选 Penpot: `python .claude/scripts/framework/setup.py --with-penpot`
- 仅检测: `python .claude/scripts/framework/setup.py --check-only`

### 升级步骤（本地路径方式）
1. 运行: `python .claude/scripts/framework/upgrade.py local <新版CataForge路径> --dry-run`
2. 确认变更列表无异常
3. 运行: `python .claude/scripts/framework/upgrade.py local <新版CataForge路径>`
...
2. 运行: `python .claude/scripts/framework/upgrade.py check` 检测新版本
3. 运行: `python .claude/scripts/framework/upgrade.py upgrade --dry-run` 预览变更
4. 运行: `python .claude/scripts/framework/upgrade.py upgrade` 执行升级
5. 检查: `git diff .claude/` 确认变更合理
6. 提交: `git commit -m "chore: upgrade CataForge framework to vX.Y.Z"`

### 独立验证
- 运行: `python .claude/scripts/framework/upgrade.py verify` 可随时检查框架文件完整性
...
**写入方式**:
- **Hook 自动**: 由 `.claude/hooks/` 中的 hook 脚本自动触发，无需 orchestrator 记忆
- **[EVENT] 手动**: 使用 `python .claude/scripts/framework/event_logger.py` CLI，已嵌入各协议步骤中（标记为 **[EVENT]**）

```bash
python .claude/scripts/framework/event_logger.py --event phase_start --phase architecture --detail "进入架构设计阶段"
```
...
| interrupt-override | Interrupt-Resume 中用户回答推翻 agent `[ASSUMPTION]` | orchestrator | hard |
| review-flag | reviewer 将 `[ASSUMPTION]` 条目判为 CRITICAL/HIGH | reviewer | review |
...
- orchestrator 命中 interrupt-override 时手动写入，其他信号勿重复记录
