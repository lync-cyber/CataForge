...
## Input Contract
- 必须加载: 通过 `python .claude/scripts/docs/load_section.py` 加载 arch 主卷: `arch#§1.4`, `arch#§6`, `arch#§7`（技术栈/目录结构/构建命名环境约定）
...
- 加载示例: `python .claude/scripts/docs/load_section.py arch#§1.4 arch#§6 arch#§7`
...
- 禁止: 跳过安全扫描
- 禁止: 修改源代码或测试
- 禁止: Bash 执行除 `python .claude/scripts/docs/load_section.py` 以及实际部署/构建命令之外的无关命令
