...
- **测试失败原因验证**: 每个 FAIL 测试必须因为"功能未实现"而失败（如 import 不存在、方法未定义、返回值不符合预期），而非因为测试自身逻辑错误（如断言字面量、语法错误、错误的测试配置）
- **断言有效性**: 每个测试必须包含至少一个与 AC 语义相关的断言（assert/expect），且断言的期望值必须从 AC 或接口契约推导，禁止硬编码无依据的魔法值
...
- 禁止: 跳过运行测试验证FAIL状态
- 禁止: 修改任何已有实现文件
- 禁止: 断言字面量（如 `assert True == False`、`expect(1).toBe(2)`）
- 禁止: 无断言测试（仅调用函数不验证返回值/副作用）
- 禁止: 断言期望值与 AC/接口契约不一致
