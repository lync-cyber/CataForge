...
## Input Contract
- 必须加载: prd — 通过 `python .claude/scripts/load_section.py` 按引用批量加载章节，典型加载顺序: §1概述 → §3非功能需求 → §2功能需求（按 F-xxx 批量查询，每批处理一组相关功能后再加载下一批）
- 禁止一次性 Read PRD 全文；覆盖所有功能点应通过 load_section.py 批量调用 `prd#§2.F-xxx` 而非 Read 整篇文件
- 可选参考: 已有技术文档、调研记录
- 加载示例: `python .claude/scripts/load_section.py prd#§1 prd#§3 prd#§2.F-001 prd#§2.F-002`
