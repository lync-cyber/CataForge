"""check_xref resolves cross-volume referents via the project-global docs root.

A split-volume main doc is invoked with ``--docs-dir=docs/<volume>/`` for
sibling resolution, but xref targets live across the whole ``docs/`` tree.
"""
...
def test_xref_resolves_sibling_tree_via_volume_docs_dir(tmp_path: Path) -> None:
    ...
    c = DocChecker("arch", str(arch), docs_dir=str(root / "docs" / "arch"), volume_type="main")
    c.check_xref()
