...
def test_registered_ingested_domain_entity_not_stale(tmp_path, capsys) -> None:
    # A registered, already-ingested DomainEntity is present on both sides of
    # the fs/kg diff — neither a stale WARN nor a missing FAIL.
    import gc
    ...
    out = capsys.readouterr().out
    assert failures == 0, out
    assert "no longer in docs" not in out, out
