...
def test_a3_trace_from_feature_reaches_modules(
) -> None:
...
def test_a15_traceability_edges_present_after_ingest() -> None:
    ...
    assert ask(
        kg.store,
        f"PREFIX cf: <{ns}> "
        "ASK { ?m cf:implements ?f . ?m a cf:Module . ?f a cf:Feature }",
    )
...
def test_a15_all_entities_enumerates_fixture_set() -> None:
    ...
    assert ids == {"F-001", "F-002", "AC-001", "AC-002", "M-001", "M-002", "TC-001", "TC-002"}
