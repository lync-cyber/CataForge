...
def test_page_extractor_pulls_route_and_layout() -> None:
    ...
    entity = ExtractedEntity(
        entity_id="P-001", class_name="Page", source_doc="ui-spec",
        source_section="P-001", content_hash="h", section_line_start=0,
        section_line_end=0, file_path=Path("x"), mtime=0.0,
    )
...
def test_page_extractor_omits_absent_labels() -> None:
    ...
    entity = ExtractedEntity(
        entity_id="P-002", class_name="Page", source_doc="ui-spec",
        source_section="P-002", content_hash="h", section_line_start=0,
        section_line_end=0, file_path=Path("x"), mtime=0.0,
    )
...
def test_task_extractor_normalizes_status() -> None:
    ...
    entity = ExtractedEntity(
        entity_id="T-001", class_name="Task", source_doc="dev-plan",
        source_section="T-001", content_hash="h", section_line_start=0,
        section_line_end=0, file_path=Path("x"), mtime=0.0,
    )
...
def test_task_extractor_drops_invalid_status() -> None:
    ...
    entity = ExtractedEntity(
        entity_id="T-002", class_name="Task", source_doc="dev-plan",
        source_section="T-002", content_hash="h", section_line_start=0,
        section_line_end=0, file_path=Path("x"), mtime=0.0,
    )
...
def test_build_entity_quads_single_valued_scalar() -> None:
    ...
    quads = build_entity_quads(
        "P-001", "Page", "Login Page", "ui-spec", "P-001", "abc123",
        "https://cataforge.dev/instance/proj-test", config,
