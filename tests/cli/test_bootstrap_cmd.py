...
class TestDryRunPlan:
    ...
    def test_fresh_install_with_platform_plans_all_steps(
        ...
        result = runner.invoke(cli, ["bootstrap", "--platform", "claude-code", "--dry-run"])
    ...
    def test_scaffold_drift_triggers_upgrade_and_deploy(
        ...
        agent = next((deployed_project / ".cataforge" / "agents").rglob("AGENT.md"))
        ...
        assert "scaffold refreshed" in result.output or "scaffold refresh required" in result.output
...
class TestExecution:
    def test_fresh_install_writes_everything(self, runner: CliRunner, fresh_project: Path) -> None:
    ...
    def test_idempotent_second_run(self, runner: CliRunner, fresh_project: Path) -> None:
    ...
    def test_dry_run_writes_nothing(self, runner: CliRunner, fresh_project: Path) -> None:
