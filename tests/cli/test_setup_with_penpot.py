...
def test_with_penpot_sets_design_tool_and_writes_spec(
    ...
    # spec carries a literal default + url_env; the token (if any) stays in env,
    # never in this git-tracked spec
    assert "url_env: PENPOT_MCP_URL" in body
    assert "http://localhost:9001/mcp/stream" in body
