...
def test_doctor_flags_unknown_field_in_event_log(tmp_path: Path, monkeypatch) -> None:
    project = _project(tmp_path)
    _write_event_log(
        project,
        [
...
def test_doctor_passes_with_clean_skill_templates(tmp_path: Path, monkeypatch) -> None:
    project = _project(tmp_path)
...
def _set_cutoff(project: Path, ts: str) -> None:
    ...
    data.setdefault("upgrade", {}).setdefault("state", {})["event_log_validate_since"] = ts
    fw.write_text(json.dumps(data) + "\n", encoding="utf-8")


def test_doctor_skips_pre_cutoff_legacy_records(tmp_path: Path, monkeypatch) -> None:
...
def test_doctor_still_fails_on_post_cutoff_invalid_records(tmp_path: Path, monkeypatch) -> None:
...
def test_doctor_hints_accept_legacy_when_no_cutoff_set(tmp_path: Path, monkeypatch) -> None:
...
def test_doctor_tolerates_malformed_cutoff(tmp_path: Path, monkeypatch) -> None:
